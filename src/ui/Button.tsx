...
function Button({
  ...
  disabled?: boolean;
  to?: string;
  type: "sm" | "primary" | "secondary" | "round";
  onClick?: (e?: React.MouseEvent<HTMLButtonElement>) => void;
}) {
  // const className = `inline-block rounded-full bg-yellow-400 px-4 py-3 font-semibold
  //     uppercase tracking-wid text-stone-800 transition-colors duration-300
  //     hover:bg-yellow-300 focus:bg-yellow-300 focus:outline-none focus:ring
  //     focus:ring-yellow-300 focus:ring-offset-2 active:bg-slate-400
  ...
  const styles = {
    ...
    round: base + " px-2.5 oy-1 md:px-3.5 md:py-2 text-sm",
    secondary: `inline-block text-sm rounded-full border-2 border-stone-300 font-semibold
    uppercase tracking-wid text-stone-400 transition-colors duration-300
    hover:bg-stone-300 hover:text-stone-800 focus:text-stone-800 focus:bg-stone-300 focus:outline-none focus:ring 
    focus:ring-stone-200 focus:ring-offset-2 active:bg-slate-400
